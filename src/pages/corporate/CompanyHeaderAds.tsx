...
export const CompanyHeaderAds: React.FC = () => {
  ...
  const fetchApps = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/company-ads/my-company-apps`, {
  ...
  const fetchCategories = async (appId: string) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await axios.get(`${API_BASE_URL}/company-ads/categories/${appId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
  ...
  const fetchCompanyAds = async () => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await axios.get(`${API_BASE_URL}/company-ads`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
        // Filter ads for selected app only
        const filteredAds = response.data.data.filter((ad: CompanyAd) => ad.app_id === parseInt(selectedAppId));
        setCompanyAds(filteredAds);
      }
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to fetch company ads');
    }
  };
  ...
  const handleSave = async (categoryId: number) => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/company-ads`, formDataToSend, {
        headers: {
      ...
      setSuccess('Company ad created successfully');
  ...
  const handleEdit = async (ad: CompanyAd) => {
    setEditingId(ad.id);
    setError('');
    setSuccess('');
  };
  ...
  const handleUpdate = async (adId: number, categoryId: number) => {
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/company-ads/${adId}`, formDataToSend, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'multipart/form-data'
        }
      ...
      setSuccess('Company ad updated successfully');
      setEditingId(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await axios.delete(`${API_BASE_URL}/company-ads/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setSuccess('Company ad deleted successfully');
      fetchCompanyAds();
    } catch (err: any) {
  ...
  const getAdForCategory = (categoryId: number): CompanyAd | undefined => {
    return companyAds.find(ad => ad.app_category_id === categoryId);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Company Header Ads Management</h1>
      </div>
      ...
      {!selectedAppId && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <p className="text-gray-500 text-lg">Please select an app to manage company header ads</p>
        </div>
      )}
    </div>
  );

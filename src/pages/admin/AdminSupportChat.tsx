...
import { MessageSquare, Send, User, Clock, RefreshCw, X, CheckCircle, XCircle } from 'lucide-react';
import { API_BASE_URL } from '../../config/api.config';

interface Conversation {
  id: number;
  subject: string;
  channel: 'admin' | 'accounts' | 'technical';
...
export const AdminSupportChat: React.FC = () => {
  ...
  const [channelFilter, setChannelFilter] = useState<'all' | 'admin' | 'accounts' | 'technical'>('all');
  ...
  const getChannelColor = (channel: string) => {
    switch (channel) {
      ...
      case 'accounts': return 'bg-orange-100 text-orange-700';
          ...
          <div className="p-3 border-b border-gray-200 space-y-2">
            ...
            <div className="flex gap-2">
              {(['all', 'admin', 'accounts', 'technical'] as const).map(c => (
                <button key={c} onClick={() => setChannelFilter(c)} className={`px-2 py-1 text-xs rounded ${channelFilter === c ? 'bg-purple-600 text-white' : 'bg-gray-100'}`}>
                  {c}
                </button>
              ))}
            </div>
            ...
            {loading ? (
              ...
              conversations.map(conv => (
                <div
                  ...
                  className={`p-3 border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${selectedConversation?.id === conv.id ? 'bg-blue-50' : ''}`}
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-500">{conv.user?.first_name} {conv.user?.last_name}</span>
                    <span className={`px-2 py-0.5 rounded text-xs ${getChannelColor(conv.channel)}`}>{conv.channel}</span>
                  </div>
